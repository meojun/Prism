#!/bin/bash
# Build the 20 canonical 4-model traces, then freeze their hashes.
#
...
fail=0
for rate in 2 4 6 8 10; do
  for seed in 1 2; do
    if [ -s "$OUT/bursty_r${rate}_s${seed}.pkl" ] && [ -s "$OUT/steady_r${rate}_s${seed}.pkl" ]; then
      echo "[4het] r${rate}_s${seed} already present"; continue
    fi
    echo "[4het] building r${rate}_s${seed}"
    $PY "$SCRIPT_DIR/build_paired_workload.py" \
      ...
      >> "$OUT/build.log" 2>&1 \
      || { echo "[4het] build FAILED for r${rate}_s${seed}" >&2; fail=1; }
  done
done
[ "$fail" = 0 ] || { echo "FATAL: a build failed; see $OUT/build.log" >&2; exit 1; }

mkdir -p "$ROOT/exp/results/4het-paired"
$PY "$SCRIPT_DIR/p4het_freeze_workloads.py" --workloads "$OUT" \
  --out "$ROOT/exp/results/4het-paired/WORKLOAD_MANIFEST.json"
