...
OUT="$ROOT/exp/results/4het-cooldown-diagnostic"
...
mkdir -p "$OUT/raw"
log() { echo "[$(date -u +%FT%TZ)] [cooldown-diag] $*" | tee -a "$OUT/pipeline.log"; }
stop_all() { bash "$SCRIPT_DIR/notify.sh" "cd-stop-$(date +%s)" \
  "⛔ STOPPED | cooldown diagnostic | $1" || true; log "STOPPED: $1"; exit 1; }
...
print(m['files']['${kind}_r${rate}_s${seed}.pkl']['sha256'])")
  ...
  d="$OUT/raw/prism-cooldown0/${kind}/rate_${rate}/seed_${seed}"
...
rec = {"idx": int(idx), "arm": "prism-cooldown0", "workload": kind,
...
log "cooldown diagnostic complete"
bash "$SCRIPT_DIR/notify.sh" "cd-done-$(date +%s)" \
  "✅ SUCCESS | cooldown diagnostic complete | 4/4" || true
exit 0
