...
step results-index $PY "$SCRIPT_DIR/final_results_index.py" \
  --root "$ROOT" --out "$ROOT/reports/prism/00_overview/FINAL_RESULTS_INDEX.md"
...
step handoff-document $PY "$SCRIPT_DIR/final_handoff_doc.py" \
  --root "$ROOT" --out "$ROOT/reports/prism/00_overview/FINAL_BASELINE_HANDOFF.md" \
...
$PY "$SCRIPT_DIR/final_handoff_doc.py" --root "$ROOT" \
  --out "$ROOT/reports/prism/00_overview/FINAL_BASELINE_HANDOFF.md" \
  --handoff-sha "$(git rev-parse --short HEAD)" >> "$HANDOFF/handoff-document.log" 2>&1 || true
if ! git diff --quiet -- reports/prism/00_overview/FINAL_BASELINE_HANDOFF.md; then
  git add reports/prism/00_overview/FINAL_BASELINE_HANDOFF.md
  git -c user.name="Prism Baseline Agent" -c user.email="causslab@gmail.com" \
...
conditions = {
    ...
    "handoff_document": (root / "reports/prism/00_overview/FINAL_BASELINE_HANDOFF.md").is_file(),
